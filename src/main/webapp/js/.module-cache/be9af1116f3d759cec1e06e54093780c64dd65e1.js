...
var defaultTableCellRenderer = function(rowIndex, colIndex, val) {
    return (
...
var defaultTableHeaderRowRenderer = function(props) {
    ...
    var headerRowClassName = props.headerRowClassName;
    var headerColumnClassName = props.headerColumnClassName;
    console.log("default header headerRowClassName: " + rowIndex + ",colIndex:" + colIndex + ",val:" + val);
    return (
...
var defaultTableCellRenderer = function(rowIndex, colIndex, val, props) {
    console.log("default cell rowIndex: " + rowIndex + ",colIndex:" + colIndex + ",val:" + val);
    return (
        React.createElement("span", null, val)
    );
};
...
var TableField = React.createClass({displayName: "TableField",
  render: function() {
    ...
    if (headerRenderer == null)
        headerRenderer = defaultTableHeaderRowRenderer;
    var cellRenderer = this.props.cellRenderer;
    if (cellRenderer == null)
        cellRenderer = defaultTableCellRenderer

    var rowClassName = this.props.rowClassName;
    var colClassName = this.props.columnClassName;
    var header = null;
    ...
    return (
        ...
        dataValue.map(function(row, index, rows)
         {
             return
             index == 0 ? headerRenderer(component.props)
             :
             (
                 React.createElement("tr", {key: index, className: rowClassName}, 
                     row.map(function(col, colIndex, cols) {
                     console.log("key: " + key + ",col:" + col + ",index:" + index + ",colIndex" + colIndex);
                     return (
...
function getDefaultValsFromSpec(theSpec)
{
    ...
    for (var index = 0; index < theSpec.length; index++) {
        ...
        var id = itemSpec.id;
        if (getProp(itemSpec, 'defaultValue') != null)
