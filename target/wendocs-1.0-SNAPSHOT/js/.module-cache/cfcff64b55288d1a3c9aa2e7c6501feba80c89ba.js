...
var TableField = React.createClass({displayName: "TableField",
  render: function() {
    ...
    var defaultTableCellRenderer = function(rowIndex, colIndex, val, props) {
        ...
        return renderBySpec(cellSpec, rowIndex * 100 + colIndex, val, props.onUserInput);
    };
...
var Form = React.createClass({displayName: "Form",
  ...
  render: function() {
    ...
    var doUpdateData = function(data, updateId, updateData)
    {
    ...
    var processDataUpdate = function(updateId, updateVal, getDeltaData) {
        var currState = component.getState();
    ...
    var getHandleTableChangeFunc = function(id) {
        var f = function(val) {
            var getDeltaData = function(data, updateId, updateVal) {
                var rowIndex = updateVal[0];
                var colIndex = updateVal[1];
                var cellVal = updateVal[2];
                var dataVal = data[updateId];
                dataVal[rowIndex][colIndex] = cellVal;
                var retval = {};
                retval[updateId] = dataVal;
                return retval;
            };
            return processDataUpdate(id, val, getDeltaData);
        };
        return f;
    };
  ...
  return (
    ...
    this.props.spec.map(
        function(itemSpec, index, arr) {
            ...
            var type = itemSpec.type;
            if (type == 'table')
                onUserInput = getHandleTableChangeFunc(itemSpec.id);
            else
                onUserInput = getHandleChangeFunc(itemSpec.id);
